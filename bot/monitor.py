...
let prevRec={},prevPos={};
function dirCls(sym,px,store){const p=store[sym];return p==null?'':(px>p?'up':px<p?'dn':'')}
...
async function tick(){
 try{const d=await(await fetch('/bot.json?t='+Date.now())).json();
  ...
  rc.forEach(r=>{prevRec[r.symbol]=r.price});
  ...
  let b=$('pos').querySelector('tbody');b.innerHTML=(d.positions||[]).map(x=>{const pd=dirCls(x.symbol,x.price,prevPos),ar=pd==='up'?' ▲':pd==='dn'?' ▼':'';return `<tr><td>${x.symbol}</td><td>${f(x.entry_price,4)}</td><td class="${pd}" style="font-weight:700">${f(x.price,4)}${ar}</td><td class=${cl(x.pnl_pct)}>${sg(x.pnl_pct)}%</td><td><button class="sell" onclick="sellPos('${x.symbol}')">بيع</button></td></tr>`}).join('')||'<tr><td colspan=5 class=mut>لا صفقات</td></tr>';
  (d.positions||[]).forEach(x=>{prevPos[x.symbol]=x.price});
